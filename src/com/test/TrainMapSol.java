...
/**
 * Given a train map with stations and connections, find the shortest path
 * between two stations.
 *
 * For example, for the input "King's Cross St Pancras" and "St Paul's", the
 * output should be "King's Cross St Pancras->Russel Square->Holborn->Chancery
 * Lane->St Paul's".
 *
 * The program includes a method to compute the shortest path and a main method
 * to test it.
 */

public class TrainMapSol {
	...
	private static class TrainMap {
		...
		public List<Station> shortestPath(String from, String to) {
			...
			Map<Station, Station> sourceDest = new HashMap<>();

			Station sourceStaion = stations.get(from);

			distance.put(sourceStaion, 0);

			while (visited.size()!=stations.size()) {

				Station stn = getMinStation(distance, visited);
				visited.add(stn);

				for (Station s : stn.neighbours) {
					if (!visited.contains(s) && distance.get(s) > distance.get(stn) + 1) {
						distance.put(s, distance.get(stn) + 1);
						sourceDest.put(s, stn);
					}
				}

			}
			List<Station> stnList = new ArrayList<>();
			Station tempStn = stations.get(to);
			stnList.add(tempStn);
			while (sourceStaion != tempStn) {
				tempStn = sourceDest.get(tempStn);
				stnList.add(tempStn);
			}
			//stnList.add(sourceStaion);
			Collections.reverse(stnList);
			return stnList;
		}

		private Station getMinStation(Map<Station, Integer> distance, Set<Station> visited) {
			...
			for (Station s : stations.values()) {
				...
				if (mindis > distance.get(s) && !visited.contains(s)) {
					st = s;
					mindis = distance.get(s);
				}
			}
			return st;
